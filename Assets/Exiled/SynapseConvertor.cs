...
public class SynapseConvertor : MonoBehaviour
{
    ...
    Dictionary<string, SchematicObjectDataList> LoadJson()
    {
        var path = Path.Combine(Application.dataPath, "SchematicsLoad");
        if (!Directory.Exists(path))
        {
            Directory.CreateDirectory(path);
            Debug.Log($"Path \"{path}\" dont exist ! path created.");
            return null;
        }
